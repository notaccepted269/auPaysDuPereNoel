...
namespace AuPaysDuPereNoel
{
    ...
    public class LinkedStack<T>
    {
        ...
        public T pop()
        {
            if (IsEmpty) throw new InvalidOperationException("Pile vide");
    ...
    public class LinkedQueue<T>
    {
        ...
        public T Dequeue()
        {
            if (IsEmpty) throw new InvalidOperationException("File vide");
   ...
   public class Entrepot
    ...
    public class Simulation // La classe qui gère la simulation.
    {
        ...
        // Files et piles
        public LinkedStack<Lettre> BureauPereNoel { get; set; }
        public LinkedQueue<Lettre> FileAttenteFabrication { get; set; }
        public LinkedQueue<Lettre> FileAttenteEmballage { get; set; }
        public LinkedQueue<Lettre>[] FilesAttenteContinents { get; set; }
        ...
        public Simulation(int nbLutins, int nbNains, int nbEnfants, int nbLettresParHeure, int nbJouetParTraineau)
        {
            ...
            BureauPereNoel = new LinkedStack<Lettre>();
            FileAttenteFabrication = new LinkedQueue<Lettre>();
            FileAttenteEmballage = new LinkedQueue<Lettre>();

            FilesAttenteContinents = new LinkedQueue<Lettre>[5];
            for (int i = 0; i < 5; i++)
            {
                FilesAttenteContinents[i] = new LinkedQueue<Lettre>();
            }
    ...
    /// Exécute une heure de simulation : arrivée de lettres, travail du personnel, voyages, coûts.
        public void AvancerUneHeure()
        {
            ...
            foreach (Elfe elfe in Elfes)
            {
                ...
                {
                    LinkedQueue<Lettre> fileContinent = FilesAttenteContinents[(int)elfe.Continent];
        ...
        public bool ToutesLettresTraitees()
        {
            ...
            foreach (LinkedQueue<Lettre> file in FilesAttenteContinents)
            {
                if (file.Count > 0)
                    return false;
            }
    ...
    /// Ajuste le nombre de lutins actifs (travail ou attente) en respectant le délai de 12 heures.
        ...
        public void ModifierNombreLutins(int nouveauNombre)
        {
            ...
            if (nouveauNombre < actifs)
            {
                ...
                LinkedStack<Lutin> disponibles = new LinkedStack<Lutin>();
                foreach (Lutin lutin in Lutins)
                {
                    if (lutin.Statut == StatutEmploye.EnAttente)
                    {
                        disponibles.Push(lutin);
                    }
                }
                ...
                // Mise en repos
                for (int i = 0; i < aMettreEnRepos; i++)
                {
                    Lutin lutin = disponibles.pop();
            ...
            else
            {
                ...
                LinkedQueue<Lutin> repos = new LinkedQueue<Lutin>();
                foreach (Lutin lutin in Lutins)
                {
                    if (lutin.Statut == StatutEmploye.EnRepos)
                    {
                ...
                // Réactivation
                for (int i = 0; i < aReactiv; i++)
                {
                    Lutin lutin = repos.Dequeue();
    ...
    /// Ajuste le nombre de nains actifs avec un délai minimum de 24 heures entre deux actions.
        ...
        public void ModifierNombreNains(int nouveauNombre)
        {
            ...
            int actifs = 0;
            foreach (Nain nainActif in Nains)
            {
                if (nainActif.Statut != StatutEmploye.EnRepos)
                {
                    actifs++;
                }
            }
            ...
            if (nouveauNombre < actifs)
            {
                ...
                LinkedStack<Nain> disponibles = new LinkedStack<Nain>();
                foreach (Nain nain in Nains)
                {
                    if (nain.Statut == StatutEmploye.EnAttente)
                    {
                        disponibles.Push(nain);
                    }
                }
            ...
            else
            {
                ...
                LinkedQueue<Nain> repos = new LinkedQueue<Nain>();
                foreach (Nain nain in Nains)
                {
